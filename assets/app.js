(function () {
  ...
  function render() {
    ...
    if (html) setContent(html, route==='/'?'AC Second Opinion':route==='/check'?'Guided AC check':route==='/result'?'Your next step':route==='/feedback'?'Beta feedback':'Beta information');
  }
  ...
  document.addEventListener('click', event => {
    ...
    if(name==='start')start(action.dataset.seed||'');
    if(name==='test')start('Hypothetical AC concern.','test');
    if(name==='stop'){
      if(!state)state=F.create();
    ...
    if(name==='new'){
      if(window.confirm('Start a new check? Save any service note first. A new check always begins with safety.'))start();
    }

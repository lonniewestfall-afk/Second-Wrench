(function () {
  ...
  function flow() {
    ...
    const node = (F.viewNode ? F.viewNode(state.node) : F.nodes[state.node]);
  ...
  const pages = {
    '/about': () => content('A second set of questions. Not a second guess.', '<p class="lead">AC Second Opinion is a structured guide for homeowners who need a clearer next step before an HVAC service call.</p><div class="document-panel"><h2>How it works</h2><p>First, tell us about any hazards. Then identify the equipment and answer one question at a time. The result explains a limited next step and creates a short note you can share with your service company.</p><h2>What makes the decisions?</h2><p>This beta uses fixed, reviewable decision rules—not an open-ended AI repair chatbot. The same answers follow the same rules. It does not inspect equipment, interpret images, measure voltage, or verify what a user reports.</p><h2>Three levels, one boundary at a time</h2><p><strong>Basic homeowner checks:</strong> thermostat Cool mode and setpoint, user-serviceable thermostat batteries, a filter you can already reach safely, and letting ice thaw with the system Off. Also: returns and supplies blocked by furniture, rugs, or closed vents; exterior debris at the outdoor unit; a panel-door look at the outdoor/AC breaker (optional reset only when it is dry and you know which breaker); and a visual look at whether the outdoor disconnect appears On or Off. That disconnect step does not tell you to flip the lever. An unusual noise starts with a hazard screen.</p><p><strong>Advanced DIY:</strong> '+(advancedOn?'enabled in this private sandbox only, and only as a gated like-for-like capacitor path after a hum with the fan not spinning. The contactor is not included. Every gate must pass. Never short a capacitor with a screwdriver or bare metal. Do not touch terminals before discharge. Covers must be on before power is restored. You can call a professional instead at every step.':'not enabled in this beta. Suspected capacitor or contactor paths say to call a professional. Owning tools or checking a box will not unlock an electrical procedure here.')+'</p><p><strong>Professional only:</strong> panel interior work, operating or opening the outdoor disconnect, refrigerant work, contactor replacement, safety-device faults, unsafe access, and problems that cannot be established by the supported observations.</p><h2>Why the small beta?</h2><p>We are testing whether the wording, boundaries, and service notes are useful. There are no payments, affiliate links, parts recommendations, or contractor bookings in this version.</p><button class="button" data-action="test">Try a test scenario '+icon('arrow')+'</button></div>'),
